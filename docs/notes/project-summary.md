...
- Coach Saved Workouts month calendar supports direct program moves: press and hold a program note, drag it over another day, and release to move that program to the target date.
...
- Athlete Training Schedule includes a month board only in phone landscape so athletes can see the full month of programs there. Portrait must keep only the compact 7-day strip and hide the month board. Tapping a day or workout selects that date/workout and keeps the detailed workout panel below in sync.

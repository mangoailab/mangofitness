...
- Programmed workouts can be marked by athletes as Done or Skipped using `athlete_workout_statuses` / `public.set_athlete_workout_status`; skipping can include a reason like vacation, sick, no equipment, or too sore. This status is separate from performance results so skipped workouts do not create fake logs.

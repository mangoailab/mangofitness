...
- Coach portal has a separate Clients page. The create form starts collapsed behind an Add Client button; edits happen inline inside each athlete card with its own Edit/Save controls. Client rows stay collapsed by default and include search to keep the page clean.

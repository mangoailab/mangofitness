...
- Coach Saved Workouts month calendar supports direct program moves in phone landscape: press and hold a program note, drag it over another day, and release to move that program to the target date. Coach portrait keeps the compact week/day view and hides the bottom month board.
...
- Cardio/WOD programming supports real-world option choices. Coaches can add multiple cardio options such as Row tester and 400m x 4. For interval work, set the Splits field to patterns like `800m x 8` or `8 x 800m`; athletes enter each split and see an auto-summed total.

...
- Programmed workouts can be marked by athletes as Done using `athlete_workout_statuses` / `public.set_athlete_workout_status`. Do not show a Skip button in the athlete UI; if a workout is not marked done, treat it as not completed/skipped. Status is separate from performance results so it does not create fake logs.

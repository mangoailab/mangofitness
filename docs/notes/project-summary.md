...
- Coach portal has a separate Clients page. The top form is for creating new athlete/client profiles; edits happen inline inside each athlete card with its own Edit/Save controls. Client rows stay collapsed by default and include search to keep the page clean.
